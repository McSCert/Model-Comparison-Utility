...
\begin{itemize}
	...
	\item \keyword{getHandle} -- Get the handle of the model element associated with the node from the comparison tree~(Section~\ref{SEC:gethandle}).
	\item \keyword{getPath} -- Get the pathname of the model element associated with the node from the comparison tree~(Section~\ref{SEC:getpath}).
	\item \keyword{plotTree} -- Plot the digraph of the comparison tree~(Section~\ref{SEC:plot}).
	\item \keyword{summaryOfChanges} -- Print a summary report of the changes in the comparison tree to the Command Window or a \file{.txt} file~(Section~\ref{SEC:summary}).
\end{itemize}
...
\begin{enumerate}
  \item Use \Matlab/\Simulink R2016b or newer.
	...
	\end{enumerate}
	\begin{itemize}
		\item \textit{Note:} If running the command ``\cmd{which find\_node}'' indicates that the script is not found, then the tool needs to be added to the \mpath.
...
\begin{figure}[!htb]
  \centering
	\includegraphics[width=\textheight, angle=90]{../figs/Plot}
...
\label{SEC:highlight}
The \func{7} function highlights model elements corresponding to nodes.
...
%\begin{lstlisting}
%>> summaryOfChanges(Edits)
%
%ChangeType, added -- TOTAL 2
%ChangeType, deleted -- TOTAL 1
...
%NodeType, block, ChangeType, added, BlockType, subsystem -- TOTAL 0
%NodeType, block, ChangeType, deleted, BlockType, subsystem -- TOTAL 0
%NodeType, block, ChangeType, renamed, BlockType, subsystem -- TOTAL 0
%NodeType, block, ChangeType, modified, BlockType, subsystem -- TOTAL 0
%NodeType, line, ChangeType, added -- TOTAL 2
%NodeType, line, ChangeType, deleted -- TOTAL 1
%NodeType, line, ChangeType, renamed -- TOTAL 2
%NodeType, line, ChangeType, modified -- TOTAL 4
%\end{lstlisting}
